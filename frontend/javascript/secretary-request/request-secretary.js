  ...
  methods: {
    async fetchRequests() {
      try {
        ...
        const response = await fetch(`${baseUrl}/secretary/${this.secretaryId}/requests`, {
    ...
    async removeConfirmed() {
      try {
        ...
        const baseUrl = window.API_BASE_URL;
        const response = await fetch(`${baseUrl}/secretary/requests/${this.selectedRequest.work_id}`, {
          method: 'DELETE',
          headers: { 'Authorization': 'Bearer ' + sessionStorage.getItem('jwt') }
        });
        if (response.ok) {
