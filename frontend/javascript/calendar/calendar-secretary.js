const calendarSecretaryApp = Vue.createApp({
    data() {
    ...
    methods: {
        ...
        async fetchApprovedLawyers() {
            ...
            try {
                ...
                const res = await fetch(`${baseUrl}/secretary-lawyers-view/${this.secretaryId}`, { headers: { 'Authorization': 'Bearer ' + sessionStorage.getItem('jwt') } });
        ...
        async openRescheduleModal(consultation) {
            ...
            try {
                const baseUrl = window.API_BASE_URL;
                const res = await fetch(`${baseUrl}/lawyer_availability/${consultation.lawyer_id || this.selectedLawyerId}`, { headers: { 'Authorization': 'Bearer ' + sessionStorage.getItem('jwt') } });
                if (res.ok) {
        ...
        async submitReschedule() {
            ...
            if (!this.validRescheduleTimes.includes(this.rescheduleTime)) {
                this.rescheduleError = `Time must be within the lawyer's office hours.`;
                this.rescheduleLoading = false;
                return;
            }
            try {
      ...
      <div class="calendar__container fadeInUp">
        <div class="calendar__panel" style="position:relative;">
          <div class="calendar__controls">
            ...
            <select id="lawyerDropdown" v-model="selectedLawyerId" @change="fetchAppointments">
...
calendarSecretaryApp.mount('.calendar');
